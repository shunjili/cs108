...
public class QuizManager {
	...
	public static ArrayList<Quiz> getQuizzesWithTag(String tag) {
		try {
			try {
				Class.forName("com.mysql.jdbc.Driver");
			} catch (ClassNotFoundException e) {
			...
			//prepare query
			String query = "SELECT DISTINCT " + TAG_QUIZ_ID_COL + " FROM " + MyDBInfo.QUIZ_TAG_TABLE + " WHERE "
					+ TAG_TAG_COL + " LIKE \"%" + tag + "%\";";
			
			ResultSet rs = stmt.executeQuery(query);
			ArrayList<String> quizIdArr = new ArrayList<String>();
			while(rs.next()) {
				quizIdArr.add(rs.getString(TAG_QUIZ_ID_COL));
			}
	...
	public static ArrayList<QuizAttempt> getTopAttempts(String quiz_id, int max) {
		try {
			...
			String query = "SELECT A.* FROM " + MyDBInfo.ATTEMPTS_TABLE + " A LEFT OUTER JOIN " + MyDBInfo.ATTEMPTS_TABLE
					...
					+ ") WHERE B." + ATTEMPT_USERNAME_COL + " IS NULL AND A." + ATTEMPT_QUIZ_ID_COL + "=" + quiz_id + " ORDER BY " + ATTEMPT_SCORE_COL + " DESC, " + ATTEMPT_DURATION_COL + " ASC LIMIT " + max + ";";
